<template>
          ...
          <h2 class="accordion-header" :id="'heading' + index">
            ...
            >
              <div class="w-100 d-flex justify-content-between align-items-center">
                ...
                <!-- Right: Status + Conditional Button -->
  ...
  onMounted(async () => {
    try {
      ...
      const candidate = userResponse.data
  
      const appResponse = await axios.get(
        `http://localhost:8000/api/applications/candidate/${candidate.id}`
      )
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(
        `http://localhost:8000/api/applications/${selectedApplication.value.id}`
      )
